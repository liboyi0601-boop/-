...
public final class TaskFeatureExtractor
{
	...
	public double[] extract(TaskCandidateView candidate, SchedulingState state)
	{
		...
		features[8] = normalizeTime(taskState == null ? 0 : taskState.getCriticalPathSlack(), workflowState, workflowWindow);
		...
		features[12] = workflowState == null ? 0.0 : workflowState.getNormalizedSlack();
		features[13] = workflowState == null ? 0.0 : cap(workflowState.getViolationRiskScore(), 10.0) / 10.0;
		features[14] = workflowState == null ? 0.0 : workflowState.getReadyTaskDensity();
		features[15] = normalizeTime(workflowState == null ? 0 : workflowState.getRemainingCriticalPathLength(),
				workflowState, workflowWindow);
